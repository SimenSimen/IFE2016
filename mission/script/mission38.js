...
(()=>{
	function init () {
		...
		function sortdata (data , n , dir) {
			var arr = [];
			var keys = Object.keys(data);
			for (var i = 0; i < keys.length; i++) {
				arr.push({name : keys[i] , data : data[keys[i]]});
			}
			arr.sort((a,b)=>{
				return (a.data[n]-b.data[n])*dir;
			});
			data = {};
			for (i = 0 ; i < arr.length; i++) {
				data[arr[i].name] = arr[i].data;
			}
			return data;
		}
		sum(data_);
		randerData(data_);
		var arrowUp = c_('arrowUp');
		var arrowDown = c_('arrowDown');
		//add handler
		for ( var i = 0; i < arrowUp.length; i++) {
			((n)=>{
				arrowUp[n].addEventListener('click' , ()=>{
					var result = sortdata(data_ , n , -1);
					randerData(result);
				});
			})(i);
		}
		for (i = 0; i < arrowDown.length; i++) {
			((n)=>{
				arrowDown[n].addEventListener('click' , ()=>{
					var result = sortdata(data_ , n , 1);
					randerData(result);
				});
			})(i);
